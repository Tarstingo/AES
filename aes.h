...
//第一个是状态文，第二个为轮密钥，plaintext最后变为plaintext加上轮密钥
void AddRoundKey(plaintext *,plaintext);

//ShiftRows操作
void ShiftRows(plaintext *);
//解密时用的ShiftRow操作的逆变换
void InvShiftRows(plaintext *);
...
//返回域上面两个元素的乘积，
byte MultiField(byte,byte);

//加密时用的MixColumn操作
void Mixcolumn(plaintext *status);

//加密时生成扩展密钥时用的RotWord行位移操作
void RotWord(word *);
...
//密钥扩充算法，主密钥key是128bit，生成初始密钥和10轮轮密钥，保存在allroundkey里面(因此allroundkey有11轮轮密钥)。
...
//加密函数,ptext明文，*allkey为由key生成的一个初始密钥和10个轮密钥,加密数据保存在*dest中
void encryptAes128(plaintext ptext,plaintext *allkey,plaintext *dest);

//解密函数，秘文为etext,allkey保存所有的轮密钥，解密数据保存到dest
